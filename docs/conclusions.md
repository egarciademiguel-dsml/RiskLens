...
**Model decisions and rationale:** [`decisions/models.md`](decisions/models.md).

---

## §1 — Horizon is the hidden variable

*Source: `horizon_crossover.ipynb`.*
...
*Source: `model_comparison.ipynb` §4b, [RL-031](../tickets/RL-031.md).*
...
*Source: `model_comparison.ipynb` §5.*
...
Persistence is tight around 0.96–0.97 for all three assets (single global fit). Crisis-regime σ_lr is 1.9–3.4× calm-regime σ_lr — the regime level differentiation is real but asset-dependent. The old "regime-sliced GARCH" failure mode (α collapsing to 0 in sparse regimes) is gone. SPY and NVDA crisis regimes produce clearly positive GPD shape (ξ > 0.5 — heavy Pareto tail), while their calm regimes are bounded. BTC is the exception: both regimes fit bounded tails, likely because the global GARCH already absorbs most of BTC's kurtosis. Full parameter tables in `decisions/models.md` §7.

**Why shared persistence.** The shared-(α, β) assumption is not a modeling compromise, it is the literature-supported best practice: Haas, Mittnik & Paolella (2004), Klaassen (2002), Marcucci (2005), and Ardia et al. (2018) all converge on intercept-only switching as the most stable specification for MS-GARCH, primarily because it avoids path-dependence (each regime's σ² at time $t$ would otherwise depend on every possible regime history up to $t$, which is computationally and statistically intractable). See `decisions/models.md` §7 for the full decision rationale.
...
Where do MS-GARCH+EVT's breaches actually happen? Tagging each backtest day with its HMM regime and splitting the breach rate shows that **breaches concentrate sharply in the crisis regime** — calm-regime breach rate is ~2% (over-conservative, the right direction), crisis-regime breach rate is ~35% (seven times the expected 5%). Per-tier comparison in §12 below.

**This is a detection lag problem, not a wrong-model problem.** The HMM needs ~5–10 days to confirm a regime switch after volatility has already changed. During that lag window the model is still predicting VaR from the old regime's parameters, and the breaches happen then. The BTC case is worse than SPY/NVDA specifically because BTC's regimes are less persistent (see §8) — the detection lag eats a larger fraction of each regime spell. The fix is structural (improve regime detection latency, or widen the transition smoothing) rather than parametric (no amount of tuning α or ξ will help). It is out of scope for RL-042.
...
*Source: `model_diagnosis.ipynb` §7, [RL-035](../tickets/RL-035.md).*
